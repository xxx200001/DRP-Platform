...
def _preprocess(raw: str) -> str:
    """半角化、OCR 标点修复、小数空格合并、序号清理、空白规整。"""
    s = raw.strip()
    s = to_halfwidth(s)
    s = s.replace("×", "x")

    # 1. 优先合并点阵 OCR/本地 OCR 拆开的小数点与空格（如 "9. 28" -> "9.28", "3. 59" -> "3.59", "6. 50-16. 00" -> "6.50-16.00"）
    #    必须先于序号剥离执行：否则 "3. 59" 会因小数点后是空格被序号正则误当成第3项剥成 "59"。
    s = re.sub(r"(?<=\d)\s*[\.．。]\s*(?=\d)", ".", s)

    # 2. 序号剥离（两条规则各管一种形态，均不许伤及真实数值）：
    #   "1. 总胆汁酸" / "2、总胆红素"  —— 点号/顿号序号。(?!\d) 确保 "4.07" 等真实数值不被误剥。
    s = re.sub(r"^\s*\d+[\.、:：](?!\d)\s*", "", s)
    #   "1 总胆汁酸 4.0" —— 裸序号后跟空格再跟指标名。
